...
class TradingBotUI {
  ...
  "comment": "TV-Signal"
            ...
            {
                title: "Market Order 2:",
                json: `{
  "account_number": "273208865",
  "symbol": "{{ticker}}",
  ...
  "volume": "0.05"
    ...
    init() {
        ...
        this.showExample(0); // Show first example by default
    ...
    updateWebhookDisplay() {
        ...
        if (webhookEndpointElement && this.webhookUrl) {
            webhookEndpointElement.textContent = this.webhookUrl;
        }
    }
...
function showExample(index) {
    if (window.ui) {
        window.ui.showExample(index);
    }
}

function copyExample() {
    if (window.ui) {
        window.ui.copyExample();
    }
}
...
document.addEventListener('keydown', (e) => {
    ...
    // Arrow keys for navigating examples
    if (window.ui && e.target.closest('.example-section')) {
        if (e.key === 'ArrowLeft') {
            e.preventDefault();
            const prevIndex = Math.max(0, window.ui.currentExampleIndex - 1);
            window.ui.showExample(prevIndex);
        } else if (e.key === 'ArrowRight') {
            e.preventDefault();
            const nextIndex = Math.min(window.ui.jsonExamples.length - 1, window.ui.currentExampleIndex + 1);
            window.ui.showExample(nextIndex);
        }
    }
});